...
This document explains the mechanisms and the rules that bound them.
`docs/lua-api-reference.md` is the *inventory*: every member, kind, constant
value, and handle method, generated by walking the live `pi` table and the host's
`impl UserData` blocks, so it cannot fall behind the surface. When the two
