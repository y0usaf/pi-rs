// Resume driver (PLAN 6.2): Pi's real SessionManager.open over the
// scenario's session fixture, restored into the real interactive
// composition with the copied renderInitialMessages/renderSessionContext
// bodies — the sdk.ts message restore (agent.state.messages =
// existingSession.messages) feeds the footer's usage/context cells, and
// populateHistory feeds the editor history.
import { mkdtempSync, readFileSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import type { AgentMessage } from "../../ref/pi/packages/agent/src/types.ts";
...
// Deterministic environment: fixed capabilities, the scenario file tree in
// a temp dir, and the scenario's static HOME (the session header carries a
// static cwd, so the footer's "~" substitution matches on both sides).
setCapabilities({ images: null, trueColor: true, hyperlinks: false });
const tempCwd = mkdtempSync(join(tmpdir(), "pi-rs-ui-parity-"));
for (const [name, contents] of Object.entries(scenario.files ?? {})) {
  writeFileSync(join(tempCwd, name), contents);
}
...
// main.ts createSessionManager: SessionManager.open(resolved path) — the
// session header's cwd becomes the effective runtime cwd.
const sessionManager = SessionManager.open(join(tempCwd, scenario.sessionFile));
...
// FooterComponent over the real session manager and the restored state.
const sessionStub = {
  get state() {
...
// interactive-mode.ts renderInitialMessages (project-trust warning not
// reachable: the fixture cwd has no trust inputs).
