...
| Tooling runtime (A.3) | Repository-owned generators, inventory/audit, and oracle tooling are Rust/Lua (`crates/pi-rs-tools`, `pi-rs-tools gate {scan,update-manifests,check}`). `nix flake check` needs no repository-owned Node/Bun/Python/shell runtime. A source-language gate rejects new first-party `.ts`/`.py`/`.sh`/shebang files and `.js` outside the browser-export allowlist; the grandfathered legacy footprint is frozen in `tests/source-language/legacy.json` and ported away over the course of A.3. Browser-export JS (export-html template + vendored highlight/marked) is the only allowed `.js`. | Difference against Pi, which is a Node/TS monorepo. pi-rs's product code is already Rust+Lua; this row extends the same target-runtime rule to the repository's own tooling so parity/audit/oracle workflows are not silently Node/Python-owned. Final. |
