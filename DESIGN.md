...
| Tooling runtime (A.3) | Repository-owned generators, inventory/audit, and oracle tooling are Rust/Lua (`crates/pi-rs-tools`, `pi-rs-tools gate {scan,update-manifests,check}`, `model-catalog {update|selftest}`, `construction-inventory {--check|--print-extracted|selftest}`). `nix flake check` needs no repository-owned Node/Bun/Python/shell runtime. The model-catalog workflow has a Rust owner replacing the former bun script; construction-inventory has a Rust owner (with offline negative-control selftest) replacing the former Python script+unittest. Inventory/audit work remains inherited until ported. A source-language gate rejects new first-party `.ts`/`.py`/`.sh`/shebang files and `.js` outside the browser-export allowlist; the grandfathered legacy footprint is frozen in `tests/source-language/legacy.json` and ported away over the course of A.3. Browser-export JS (export-html template + vendored highlight/marked) is the only allowed `.js`. | Difference against Pi, which is a Node/TS monorepo. pi-rs's product code is already Rust+Lua; this row extends the same target-runtime rule to the repository's own tooling so parity/audit/oracle workflows are not silently Node/Python-owned. Final. |
...
| Syntax highlighting | highlight.js 10.7.3 ported as a Rust mechanism binding (`pi.hljs.*`): the parse engine interprets grammar *data* compiled by the vendored library itself (`scripts/hljs-grammars` → `crates/pi-rs-host/data/hljs-grammars.json`), pinned by a vendored-library oracle. Pi's own layer—`renderHighlightedHtml`, theme mapping, `highlightCode` policy—stays Lua. All 191 highlight.js 10.7.3 registered grammars are serialized by `scripts/hljs-grammars`; 190 land in the catalog (one excluded: mathematica has a filter callback the Rust engine does not implement). The PLAN 11 grammar gap is closed. | Same split as jsdiff: the library is third-party mechanism, grammars are catalog data (never hand code), presentation is Lua. |
...
| Shared-policy substrate pack (`agent-core`) | Cross-pack agent-policy fragments that two or more product packs share (message conversion, branch-summary, compaction, system-prompt, session-runtime, bash-executor) are owned exactly once by an always-loaded `agent-core` core pack, exposed as `pi.agent.*` exact-version modules; the interactive and coding-agent packs `require` + rebind the same closures instead of concatenating chunk-local copies | A VM-wide exact-version module is single-ownership by construction (`module.define` rejects re-definition). Concatenating the same fragment into two packs would both duplicate behavior and double-define the module. Making the shared policy a non-suppressible core substrate (plan 9.10: ``Do not force singular mechanisms into ceremonial registries'') keeps per-package ablation meaningful while preserving an irreducible shared layer. |
...
| Bun/native bootstrap (`restore-sandbox-env.ts`, `windows-self-update.ts`) | Not ported — DESIGN platform boundary. `restoreSandboxEnv` recovers `process.env` from `/proc/self/environ` against a Bun compiled-binary bug (`process.env` empty under `nono`); a native Rust binary reads the real environment, so `std::env` has no such failure mode (the same rationale is already recorded in `registry/env_api_keys.rs`). `quarantineWindowsNativeDependencies` / `cleanupWindowsSelfUpdateQuarantine` move locked `.node` native addons aside so an npm/`pi update --self` reinstall on Windows can replace them; pi-rs ships no self-update or bundled native addons (see `config.rs`: "self-update half of the spec file is npm machinery with no Rust analogue"), so there is nothing to quarantine. | Darwin/`darwin-modifiers.node` and Windows console-mode binary helpers are likewise not embedded: on a pi-rs binary those helpers are exactly as unavailable as a Pi install where the addon could not be loaded, which the platform-modifiers / platform-clipboard oracles capture. |
...
| Spatiotemporal composability (2026-08-07) | In-process composition unit is the kernel `Component` (declared `reads` + effects returning `Inverse`), mounted/unmounted on a host-owned, keyed `Context`. The revert mechanism is inverse replay in reverse registration order (temporal axis) plus declared-reader notification on a committed `set` (spatial axis); atomic reload is `build → activate → publish → drain`, where publish is a single swap, not a rollback. The kernel `pi-rs-kernel` and the per-crate scope modules (host, app, session, agent, tui) landed in phase 1 of the migration; wiring the real product paths (host VM/process/session, session manager, agent seam, TUI render) is in progress under this fleet. | Axioms 02 ("snapshot in, actions out"), 05 ("one declaration mechanism"), and 06 ("bare core must boot") already made the lifecycle the substrate; this records the *backing mechanism* so ownership and lifecycle are explicit and uniform. `Context` is host-owned keyed state and the single write path; a `Component` declares reads and commits effects that each return an inverse, so an unmount restores pre-mount state with no residue. Recorded so the composition boundary is named in DESIGN, per the fleet contract; reasons and mechanism only, canon restated elsewhere. |
