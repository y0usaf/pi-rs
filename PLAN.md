...
- [ ] **8. Complete coding-agent AI/auth compatibility.** Port the providers
      ...
      **Remaining:** add deterministic differential replays + dispatch for
      `google-generative-ai`, `google-vertex`, `mistral-conversations`, and
      `bedrock-converse-stream`; upgrade the existing `openai-completions` fixtures
      to a Pi-derived differential oracle, then run the whole catalog/auth acceptance
      gate.
