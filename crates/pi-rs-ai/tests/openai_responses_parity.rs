//! Differential OpenAI Responses protocol replay (PLAN item 8).
#![allow(clippy::unwrap_used, clippy::expect_used, clippy::panic)]

use pi_rs_ai::protocols::openai_responses::{
    OpenAIResponsesOptions, ReasoningSummary, stream_openai_responses,
...
fn options(value: &Value) -> OpenAIResponsesOptions {
...
async fn run(case: &Value, models: &Value, shared: &Value) -> Value {
    ...
    let stream = if case.get("simple").and_then(Value::as_bool).unwrap_or(false) {
        match stream_simple_openai_responses(
            &model,
            &context,
        ...
        stream_openai_responses(&model, &context, Some(options(opts)))
    };
...
#[tokio::test]
async fn pi_rs_matches_pi_openai_responses_oracle() {
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/openai-responses-parity");
    let cases: Value =
