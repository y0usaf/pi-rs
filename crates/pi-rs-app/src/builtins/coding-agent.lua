...
pi.register_role({
  ...
  handler = function(args, ctx)
    ...
    local function execute_rpc_bash(cmd, session)
      ...
      local signal = pi.abort_signal()
      local result = EXTENSION_POLICY.bash_executor.execute_bash_with_operations(
        command, session:get_cwd(),
      ...
      -- stream (with `cancelled`/`truncated`/`fullOutputPath`), whereas Pi's
      ...
      -- to `stdout` and `stderr` stays "" (pi-rs does not split streams), with
      -- a real exitCode on success and "" on cancellation like Pi's
      -- session.executeBash rejection path.
      return {
        exitCode = result.exitCode or (result.cancelled and 0) or 1,
    ...
    local function do_async_command(cmd_id, cmd_type, cmd)
      ...
      elseif cmd_type == "bash" then
        ...
        local result = scripted and scripted_bash_result or execute_rpc_bash(cmd, session)
