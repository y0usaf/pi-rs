...
| `interactive-mode.ts` key handlers (`setupKeyHandlers`, `handleCtrlC/D/Z`, `handleFollowUp`, `handleDequeue`, `handleClipboardImagePaste`, `setupExtensionShortcuts`, `openExternalEditor`) | `builtins/interactive.lua` | exact escape-abort/queue-restore, press-again exit, process-group suspend + Windows status, thinking cycle/visibility toggle, follow-up/dequeue, paste insert, shortcut routing, session actions, bash-mode branches, and main/ExtensionEditor inherited external-editor handoff |
...
| `core/footer-data-provider.ts` | `builtins/interactive.lua` (+ `pi-rs-host/src/git.rs`) | exact model/context/usage adapters; git-branch discovery/watcher ported (`pi.git.current_branch`, `footer_live_branch`) and pinned against `tests/footer-git-parity/oracle.json`; extension statuses/session/provider breadth now wired via `ctx.ui.setStatus` and the live adapters |
| `interactive-mode.ts` lifecycle | `builtins/interactive.lua`, `pi-rs-tui/src/process.rs` | start/input/resize/signals/stream concurrency/cleanup, inherited child terminal release/reacquire, and SIGTSTP/SIGCONT suspension with SIGINT suppression |
