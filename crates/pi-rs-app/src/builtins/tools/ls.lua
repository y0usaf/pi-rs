...
local function format_ls_call(args, theme, base_cwd)
  ...
  local raw_path = ""
  if args ~= nil then raw_path = render.str(args.path) end
  local path_display = render.render_tool_path(raw_path, theme, base_cwd, { emptyFallback = "." })
...
local function format_ls_result(result, options, theme, show_images)
  ...
  if entry_limit or (truncation and truncation.truncated) then
    local warnings = {}
    if entry_limit then warnings[#warnings + 1] = prelude.fmt_num(entry_limit) .. " entries limit" end
...
pi.register_tool({
  ...
  ):format(LS_DEFAULT_LIMIT, truncate.DEFAULT_MAX_BYTES // 1024),
  ...
  execute = function(_tool_call_id, params, signal)
    ...
    local dir_path = path_utils.resolve_to_cwd((path ~= nil and path ~= "") and path or ".")
    ...
    local truncation = truncate.truncate_head(raw_output, { maxLines = MAX_SAFE_INTEGER })
    ...
    if truncation.truncated then
      notices[#notices + 1] = ("%s limit reached"):format(truncate.format_size(truncate.DEFAULT_MAX_BYTES))
  ...
  renderCall = function(args, theme, context)
    return render.text_component(format_ls_call(args, theme, context.cwd), 0, 0)
  end,
  renderResult = function(result, options, theme, context)
    return render.text_component(format_ls_result(result, options, theme, context.showImages), 0, 0)
  end,
})
end
