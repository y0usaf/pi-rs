...
fn fixture() -> Value {
    serde_json::from_str(include_str!(
        "../../../tests/config-settings-parity/oracle.json"
    ))
    .unwrap()
}
...
fn manager_for(global: &Value, project: &Value, project_trusted: bool) -> SettingsManager {
    let global_decl = create_config_lua(global);
...
fn run_getters(m: &SettingsManager) -> Map<String, Value> {
    ...
    insert(
        &mut out,
        "getDefaultProvider",
        opt_str(m.get_default_provider()),
    );
    ...
    insert(
        ...
        opt_str(m.get_shell_command_prefix()),
    );
    ...
    insert(
        &mut out,
        "getSteeringMode",
        Value::String(m.get_steering_mode()),
    );
    insert(
        &mut out,
        "getFollowUpMode",
        Value::String(m.get_follow_up_mode()),
    );
    insert(&mut out, "getTransport", Value::String(m.get_transport()));
    insert(
        &mut out,
        "getDefaultProjectTrust",
        Value::String(m.get_default_project_trust().to_owned()),
    );
    insert(
        &mut out,
        "getDoubleEscapeAction",
        Value::String(m.get_double_escape_action()),
    );
    insert(
        &mut out,
        "getTreeFilterMode",
        Value::String(m.get_tree_filter_mode()),
    );
    insert(
        &mut out,
        "getCodeBlockIndent",
        Value::String(m.get_code_block_indent()),
    );
    ...
    insert(
        ...
        Value::Bool(m.get_compaction_enabled()),
    );
    insert(
        &mut out,
        "getBranchSummarySkipPrompt",
        Value::Bool(m.get_branch_summary_skip_prompt()),
    );
    insert(
        &mut out,
        "getRetryEnabled",
        Value::Bool(m.get_retry_enabled()),
    );
    insert(
        &mut out,
        "getHideThinkingBlock",
        Value::Bool(m.get_hide_thinking_block()),
    );
    insert(
        &mut out,
        "getQuietStartup",
        Value::Bool(m.get_quiet_startup()),
    );
    insert(
        &mut out,
        "getCollapseChangelog",
        Value::Bool(m.get_collapse_changelog()),
    );
    insert(
        &mut out,
        "getEnableInstallTelemetry",
        Value::Bool(m.get_enable_install_telemetry()),
    );
    insert(
        &mut out,
        "getEnableSkillCommands",
        Value::Bool(m.get_enable_skill_commands()),
    );
    insert(&mut out, "getShowImages", Value::Bool(m.get_show_images()));
    insert(
        &mut out,
        "getClearOnShrink",
        Value::Bool(m.get_clear_on_shrink()),
    );
    insert(
        &mut out,
        "getShowTerminalProgress",
        Value::Bool(m.get_show_terminal_progress()),
    );
    insert(
        &mut out,
        "getImageAutoResize",
        Value::Bool(m.get_image_auto_resize()),
    );
    insert(
        &mut out,
        "getBlockImages",
        Value::Bool(m.get_block_images()),
    );
    insert(
        &mut out,
        "getShowHardwareCursor",
        Value::Bool(m.get_show_hardware_cursor()),
    );
    ...
    insert(
        ...
        json_u64(m.get_compaction_reserve_tokens()),
    );
    insert(
        &mut out,
        "getCompactionKeepRecentTokens",
        json_u64(m.get_compaction_keep_recent_tokens()),
    );
    insert(
        &mut out,
        "getImageWidthCells",
        json_u64(m.get_image_width_cells()),
    );
    insert(
        &mut out,
        "getEditorPaddingX",
        json_u64(m.get_editor_padding_x()),
    );
    insert(
        &mut out,
        "getAutocompleteMaxVisible",
        json_u64(m.get_autocomplete_max_visible()),
    );
    insert(
        &mut out,
        "getHttpIdleTimeoutMs",
        json_u64(m.get_http_idle_timeout_ms().unwrap_or(u64::MAX)),
    );
    insert(
        &mut out,
        "getWebSocketConnectTimeoutMs",
        m.get_websocket_connect_timeout_ms()
            .ok()
            .flatten()
            .map(json_u64)
    ...
    insert(
        &mut out,
        "getSkillPaths",
        Value::Array(m.get_skill_paths().into_iter().map(Value::String).collect()),
    );
    insert(
        &mut out,
        "getPromptTemplatePaths",
        Value::Array(
            m.get_prompt_template_paths()
                .into_iter()
                .map(Value::String)
                .collect(),
        ),
    ...
    insert(
        &mut out,
        "getThemePaths",
        Value::Array(m.get_theme_paths().into_iter().map(Value::String).collect()),
    );
    insert(
        &mut out,
        "getNpmCommand",
    ...
    insert(
        &mut out,
        "getThinkingBudgets",
        m.get_thinking_budgets().unwrap_or(Value::Null),
    );
...
fn keybinding_declarations_migrate_like_pi() {
    ...
    for case in oracle["keybindings"]["migrate"].as_array().unwrap() {
        ...
        let snapshot = pi_rs_host::config::evaluate(&key_decl_source(&raw), "config.lua").unwrap();
